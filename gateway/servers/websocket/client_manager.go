...
func (manager *ClientManager) GetUserList(appID uint32) (userList []string) {
	...
	global.GVA_LOG.Infof("GetUserList len:", len(manager.Users))
	return
}
...
func (manager *ClientManager) EventRegister(client *Client) {
	...
	global.GVA_LOG.Infof("EventRegister 用户建立连接 %v", client.Addr)

	if client.ProtocType == 0 {
		ConnSucc := models.ConnSucc{
			ProtoNum: models.F_Conn_Succ,
			CurrTime: time.Now().Unix(),
			Desc:     "连接成功",
		}
		responseHead := models.NewResponseHead("", "1", common.WebOK, "", ConnSucc)
		headByte, _ := json.Marshal(responseHead)
		...
		fConnSuccMarshal, _ := proto.Marshal(&fConnSucc)
		client.Send <- fConnSuccMarshal
	}
...
func (manager *ClientManager) EventUnregister(client *Client) {
	...
	global.GVA_LOG.Infof("EventUnregister 用户断开连接 Addr{%v}:AppID{%v}:UserID{%v}", client.Addr, client.AppID, client.UserID)
	if client.UserID != "" {
		//orderID := helper.GetOrderIDTime()
...
func ClearTimeoutConnections() {
	...
	global.GVA_LOG.Infof("定时清理超时连接 前当前用户数 %v ", len(clients))
	for client := range clients {
		if client.IsHeartbeatTimeout(currentTime) {
			global.GVA_LOG.Infof("心跳时间超时 关闭连接 %v UserID:{%v} %v %v", client.Addr, client.UserID, helper.TimeIntToStr(int64(client.LoginTime)), helper.TimeIntToStr(int64(client.HeartbeatTime)))
			clientManager.Unregister <- client
			_ = client.Socket.Close()
		} else {
			global.GVA_LOG.Infof("uid 存活 昵称：{%v} 上次心跳时间 %v HeartbeatTime %v currTime:%v", client.UserID, client.Nickname, helper.TimeIntToStr(int64(client.HeartbeatTime)), helper.TimeIntToStr(time.Now().Unix()))
		}
	}
	global.GVA_LOG.Infof("定时清理超时连接 后当前用户数 %v ", len(clients))
}

func PrintCurrUser() {
	clients := clientManager.GetClients()
	for client := range clients {
		global.GVA_LOG.Infof("当前用户总数:%v ,我是谁UserID:%v Addr:%v", len(clients), client.UserID, client.Addr)
	}
...
func GetUserList(appID uint32) (userList []string) {
	global.GVA_LOG.Infof("获取全部用户 %v", appID)
...
func AllSendMessages(appID uint32, userID string, data string) {
	global.GVA_LOG.Infof("全员广播ppID:%v, userID:%v, data:%v", appID, userID, data)
