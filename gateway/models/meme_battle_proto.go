...
type MatchSuccResp struct {
	...
	RoomCom
	TavernRoomUserList []MemeRoomUser `json:"room_user_list,omitempty"` //
}
...
type LoserInfo struct {
	UserId       string       `json:"user_id"`                 //被质疑者id
	IsHaveBullet bool         `json:"is_have_bullet"`          //是否有子弹 true 有子弹
	HandarmState HandarmState `json:"handarm_state,omitempty"` //骗子酒馆 用户的子弹
}
...
type LoadMsg struct {
	...
	LikeCards      []*LikeCard     `json:"like_cards,omitempty"`
...
type JoinRoomMsg struct {
	ProtoNum  string `json:"proto_numb"`
	Timestamp int64  `json:"timestamp"`
	//UserId       string           `json:"user_id"`                  //谁加入房间了
	//RoomNo       string           `json:"room_no"`                  //房间编号
	RoomCom
	RoomUserList []MemeRoomUser `json:"room_user_list,omitempty"` //
}
...
// FraudCardMsg 骗子牌
type FraudCardMsg struct {
	ProtoNum  string `json:"proto_numb"`
	Timestamp int64  `json:"timestamp"`
	Card      *Card  `json:"card"`    //房间编号
	UserId    string `json:"user_id"` //下一个 出牌用户
}
