...
import (
	...
	"slot_server/lib/utils/env"
)

// Viper
// 优先级: 命令行 > 环境变量 > 默认值
func Viper(path ...string) *viper.Viper {
	global.SvName = "slot_server"
	...
	v.OnConfigChange(func(e fsnotify.Event) {
		...
		if global.GVA_CONFIG.Mysql.LogMode != logMode {
			...
			global.GVA_SLOT_SERVER_DB.Logger = global.GVA_SLOT_SERVER_DB.Logger.LogMode(level)
			//if global.GVA_USER_DB != global.GVA_READ_DB {
			//	global.GVA_READ_DB.Logger = global.GVA_READ_DB.Logger.LogMode(level)
			//}
		}
	})
